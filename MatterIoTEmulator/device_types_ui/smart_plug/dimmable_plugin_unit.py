...
class DimmablePluginUnit(BaseDeviceUI):
    """
    DimmablePluginUnit device UI controller represent some attribues, clusters
    and endpoints corresspoding to Matter Specification v1.2
    """

    def __init__(self, parent) -> None:
        """
        Create a new `DimmablePluginUnit` UI.
        :param parent: An UI object load DimmablePluginUnit device UI controller.
        """
        ...
        self.lbl_main_status.setText('Smart plug On\n100%')
        ...
        try:
            ...
            if device_status['status'] == 'OK':
                ...
                self.lbl_main_status.setText(
                    'Smart plug {}\n{}%'.format(
                        "On" if self.on_off else "Off", round(
                            self.level / 2.54)))
        except Exception as e:
