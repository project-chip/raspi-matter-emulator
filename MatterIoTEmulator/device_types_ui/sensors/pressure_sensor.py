...
class PressureSensor(BaseDeviceUI):
    """
    PressureSensor device UI controller represent some attribues, clusters
    and endpoints corresspoding to Matter Specification v1.2
    """

    def __init__(self, parent) -> None:
        """
        Create a new `PressureSensor` UI.
        :param parent: An UI object load PressureSensor device UI controller.
        """
        ...
        pix = QPixmap(DEVICE_RESOURCE_PATH + 'pressure-sensor.png')
        ...
        self.lbl_main_status_pressure.setStyleSheet("font-size: 14pt;")
        ...
        self.line_edit_pres.textEdited.connect(self.on_text_edited)
        ...
        logging.debug("Init pressure sensor done")
    ...
    def set_initial_value(self):
        ...
        """
        try:
            data = {'pressureValue': self.pressure}
    ...
    def on_text_edited(self):
        ...
        when line edit pressure measurement is editting"""
        self.is_edit = False

    def on_return_pressed(self):
        """
        Handle update all pressure measurement attributes
        to matter device(backend) through rpc service
        after enter value to line edit done
        """
        try:
            value_press = round(float(self.line_edit_pres.text()) * 10)
            if 0 <= value_press <= 32767:
                data = {'pressureValue': value_press}
                self.client.set(data)
                self.is_edit = True
            else:
                self.message_box(ER_PRES)
                self.line_edit_pres.setText(str(self.pressure))
        except Exception as e:
    ...
    def message_box(self, message):
        ...
        msgBox.setWindowTitle("Pressure Sensor")
    ...
    def on_value_status_changed(self):
        """
        Update pressure value to matter device(backend)
        through rpc service
        """
        # logging.info('on_value_status_changed')
        self.is_edit = True
        value = random.randrange(0, 32767, 1)
        self.mutex.acquire(timeout=1)
        self.client.set({'pressureValue': value})
        self.mutex.release()
