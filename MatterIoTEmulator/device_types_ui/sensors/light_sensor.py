...
class LightSensor(BaseDeviceUI):
    """
    LightSensor device UI controller represent some attribues, clusters
    and endpoints corresspoding to Matter Specification v1.2
    """

    def __init__(self, parent) -> None:
        """
        Create a new `LightSensor` UI.
        :param parent: An UI object load LightSensor device UI controller.
        """
        ...
        pix = QPixmap(DEVICE_RESOURCE_PATH + 'light-sensor.png')
        ...
        # Light sensor status
        self.lbl_main_status_light = QLabel()
        self.lbl_main_status_light.setStyleSheet("font-size: 13pt;")
        self.lbl_main_status_light.setText('Illuminance  :')
        self.lbl_main_status_light.setAlignment(Qt.AlignCenter)
        self.parent.ui.lo_controller.addWidget(self.lbl_main_status_light)

        self.line_edit_light = QLineEdit()
        self.validator = QIntValidator()
        self.double_validator = QDoubleValidator()
        self.line_edit_light.setValidator(self.validator)
        self.line_edit_light.setValidator(self.double_validator)
        self.line_edit_light.setMaximumSize(QSize(65, 20))
        self.lbl_measure = QLabel()
        self.lbl_measure.setText('Mlx')
        self.grid_layout_light = QHBoxLayout()
        self.grid_layout_light.setAlignment(Qt.AlignCenter)
        self.grid_layout_light.addWidget(
            self.lbl_main_status_light,
            alignment=Qt.AlignRight)
        self.grid_layout_light.addWidget(
            self.line_edit_light, alignment=Qt.AlignRight)
        self.grid_layout_light.addWidget(
            self.lbl_measure, alignment=Qt.AlignRight)

        self.line_edit_light.textEdited.connect(self.on_text_edited)
        self.line_edit_light.returnPressed.connect(self.on_return_pressed)
        self.parent.ui.lo_controller.addLayout(self.grid_layout_light)
        ...
        logging.debug("Init light sensor done")
    ...
    def set_initial_value(self):
        ...
        """
        try:
            data = {'illuminanceValue': self.illuminance}
    ...
    def on_text_edited(self):
        ...
        when line edit illuminance measurement is editting"""
        self.is_edit = False

    def on_return_pressed(self):
        """
        Handle update all illuminance measurement attributes
        to matter device(backend) through rpc service
        after enter value to line edit done
        """
        try:
            illu_value = round(float(self.line_edit_light.text()), 2)
            if illu_value > 0:
                self.value_light = round(1 + 10000 * math.log(illu_value, 10))
                if self.value_light <= 65535:
                    data = {'illuminanceValue': self.value_light}
                    self.client.set(data)
                    self.is_edit = True
                else:
                    self.message_box(ER_LIGHT)
                    self.line_edit_light.setText(str(self.illuminance))
            else:
                self.message_box(ER_LIGHT)
        except Exception as e:
    ...
    def message_box(self, message):
        ...
        msgBox.setWindowTitle("Light Sensor")
    ...
    def on_device_status_changed(self, result):
        ...
        try:
            ...
            illuminance_measured_value = device_status['reply'].get(
                'illuminanceValue')
            self.illuminance = round(
                (10 ** ((illuminance_measured_value - 1) / 10000)), 2)
            if self.is_edit:
                self.line_edit_light.setText(str(self.illuminance))
    ...
    def on_value_status_changed(self):
        """
        Update illuminance value to matter device(backend)
        through rpc service
        """
        ...
        self.client.set({'illuminanceValue': value})
        self.mutex.release()
