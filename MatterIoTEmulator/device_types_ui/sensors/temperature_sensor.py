...
class TemperatureSensor(BaseDeviceUI):
    """
    TemperatureSensor device UI controller represent some attribues, clusters
    and endpoints corresspoding to Matter Specification v1.2
    """

    def __init__(self, parent) -> None:
        """
        Create a new `TemperatureSensor` UI.
        :param parent: An UI object load TemperatureSensor device UI controller.
        """
        ...
        self.temperature = 2560

        # Show icon
        self.lbl_main_icon = QLabel()
        pix = QPixmap(DEVICE_RESOURCE_PATH + 'temperature-sensor.png')
        ...
        self.lbl_main_status_temperature.setStyleSheet("font-size: 13pt;")
    ...
    def set_initial_value(self):
        ...
        """
        try:
            data = {'temperatureValue': self.temperature}
            self.client.set(data)
        except Exception as e:
            self.parent.wkr.connect_status.emit(STT_RPC_INIT_FAIL)
            print("can not set initial value: " + str(e))

    def on_text_edited(self):
        """Enable 'is_edit_temp' attribute
        when line edit temperature measurement is editting"""
        self.is_edit_temp = False

    def on_return_pressed(self):
        """
        Handle update all temperature measurement attributes
        to matter device(backend) through rpc service
        ...
        try:
            ...
            if 0 <= value_temp <= 10000:
                data = {'temperatureValue': value_temp}
                self.client.set(data)
                self.is_edit_temp = True
    ...
    def message_box(self, message):
        ...
        msgBox.setWindowTitle("Temperature SenSor")
    ...
    def on_value_status_changed(self):
        """
        Update temperature value to matter device(backend)
        through rpc service
        """
        ...
        self.client.set({'temperatureValue': value})
        self.mutex.release()
