...
class HumiditySensor(BaseDeviceUI):
    """
    HumiditySensor device UI controller represent some attribues, clusters
    and endpoints corresspoding to Matter Specification v1.2
    """

    def __init__(self, parent) -> None:
        """
        Create a new `HumiditySensor` UI.
        :param parent: An UI object load HumiditySensor device UI controller.
        """
        super().__init__(parent)
        self.humidity = 500
        # Show icon
        self.lbl_main_icon = QLabel()
        pix = QPixmap(DEVICE_RESOURCE_PATH + 'humidity-sensor.png')
        ...
        self.lbl_main_status_humidity.setStyleSheet("font-size: 13pt;")
        self.lbl_main_status_humidity.setText('Humidity :')
        ...
        logging.debug("Init Humidity sensor done")

    def on_text_edited_hum(self):
        """Enable 'is_edit_hum' attribute
        when line edit humidity measurement is editting"""
        self.is_edit_hum = False

    def on_return_pressed(self):
        """
        Handle update all humidity measurement attributes
    ...
    def message_box(self, message):
        ...
        msgBox.setWindowTitle("Humidity Sensor")
    ...
    def set_initial_value(self):
        ...
        """
        try:
            data = {'humidityValue': self.humidity}
            self.client.set(data)
        except Exception as e:
    ...
    def on_device_status_changed(self, result):
        ...
        try:
            ...
            self.humidity = round(
                float(
                    device_status['reply'].get('humidityValue') /
                    100.0),
                2)
            if self.is_edit_hum:
                self.line_edit_hum.setText(str(self.humidity))
    ...
    def on_value_status_changed(self):
        """
        Update humidity sensor value to matter device(backend)
        through rpc service
        """
        # logging.info('on_value_status_changed')
        value = random.randrange(0, 10000, 1)
        self.is_edit_hum = True
        self.mutex.acquire(timeout=1)
        self.client.set({'humidityValue': value})
        self.mutex.release()
