...
class ContactSensor(BaseDeviceUI):
    """
    ContactSensor device UI controller represent some attribues, clusters
    and endpoints corresspoding to Matter Specification v1.2
    """

    def __init__(self, parent) -> None:
        """
        Create a new `ContactSensor` UI.
        :param parent: An UI object load ContactSensor device UI controller.
        """
        super().__init__(parent)
        # Show icon
        self.lbl_main_icon = QLabel()
        pix = QPixmap(DEVICE_RESOURCE_PATH + 'contact-sensor-2.png')
        ...
        self.lbl_main_status_contact.setStyleSheet("font-size: 13pt;")
        self.lbl_main_status_contact.setText('Status: Close')
    ...
    def on_device_status_changed(self, result):
        ...
        try:
            ...
            if device_status['status'] == 'OK':
                self.contact_value = device_status['reply'].get('booleanState')
                if str(self.contact_value) == 'False':
                    self.lbl_main_status_contact.setText('Status: Open')
                else:
                    self.lbl_main_status_contact.setText('Status: Close')
    ...
    def on_value_status_changed(self):
        """
        Update contact sensor value to matter device(backend)
        through rpc service
        """
        # logging.info('on_value_status_changed')
        self.contact_value = not self.contact_value
        self.mutex.acquire(timeout=1)
        self.client.set({'booleanState': self.contact_value})
        self.mutex.release()
